...
// 	// AddToKqueue(event, kq, client_fd, EVFILT_TIMER, EV_ADD | EV_ENABLE);
// 	AddToKqueue(event, kq, client_fd, EVFILT_WRITE, EV_ADD | EV_DISABLE);
// 	EV_SET(&event, client_fd, EVFILT_TIMER, EV_ADD | EV_ENABLE, NOTE_SECONDS, 10, NULL);
// 	if (kevent(kq, &event, 1, NULL, 0, NULL) == -1)
// 	{
// 		remove_client(client_fd);
// 		std::cout << "\033[31mkevent failed for fd " << client_fd << ": " << strerror(errno) << "\033[0m" << std::endl;
// 	}

// 	Request * new_request = new Request(client_fd, ReadHeader, ConfigPars, server_port);
// 	std::cout << "[" << client_fd << "]" << std::endl;
// 	new_request->SetTimeOut(std::time(0));
...
// 	{
// 		std::cout << "Read " << client_fd << std::endl;
// 		request->SetTimeOut(std::time(0));
...
// 			else
// 			{
// 				std::cout << "chnge to write" << std::endl;
// 				struct kevent ev;
...
// 	{
// 		std::cout << "Wirte " << client_fd << std::endl;
// 		request->SetTimeOut(std::time(0));
...
// 	{
// 		std::cout << "TIMES IS UP FOR: " << client_fd << std::endl;
// 		time_t currentTime = time(0);
// 		std::cout << "--0 " << currentTime - request->GetTimeOut() << std::endl;
// 		// req.GetHeaderValue("connection") == "keep-alive"
// 		if(currentTime - request->GetTimeOut() >= 10)
// 		{
// 			std::cout << "+" << response->_cgi.gethasPendingCgi() << std::endl;
// 			if (response->_cgi.gethasPendingCgi() || !request->GetHeaderBuffer().empty())
// 			{
...
// 					}
// 					int tempClientFd = response->getClientFd();
